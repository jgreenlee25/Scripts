...
def update_ms_servers_overrides(site, version, branch=None):
    print
    print "Updating ms-servers-overrides.json...\n"

    if branch is None:
        script_dir = MARKETLIVE_HOME + "/sites/" + site + "/trunk/source/ant/"
    else:
        script_dir = MARKETLIVE_HOME + "/sites/" + site + "/branches/" + branch + "/source/ant/"

    infile = open(script_dir+"ms-servers-overrides.json")
    data = infile.read().replace("mldev", site).replace("\"database\" : \"master\"", "\"database\" : \"master-" + site +"\"").replace("\"env\" : \"review\"", "\"env\" : \"revv"+version+"\"").replace("\"database\" : \"review\"", "\"database\" : \"review-" + site+"\"")
    infile.close()

    with open(script_dir + "ms-servers-overrides.json", "w") as outfile:
        outfile.write(data)

    print bcolors.OKGREEN+"ms-servers-overrides.json updated..."+bcolors.ENDC
...
if __name__=="__main__":
    ...
    if command == "options":
		...
		if num_args == 3:
		   ...
		   update_ms_servers_overrides(site, version, None)
		else:
		   site = sys.argv[2]
		   branch = sys.argv[3]
		   version = get_site_version(site)
		   update_ms_servers_overrides(site, version, branch)
        ...
        # Build trunk
            ...
            setup_tomcat(site)
            setup_snapshots(site)
            update_ms_servers_overrides(site, version)
            refresh_mongo(site)
            deploy_site(site)
